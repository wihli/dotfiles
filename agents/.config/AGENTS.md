...
- **Calibrate technical explanations for a reader who can follow details but may not retain domain terminology.** Lead with the concrete outcome and safety reason; keep meaningful system nouns (for example, a load-balancer listener or IAM permissions), and briefly define unfamiliar terms inline or link a reference. Do not open with abstract framework labels such as "module-wide dependency."
...
- **Write for a reader with zero context on this codebase, not "a reviewer."** Ground a technical claim in an observable symptom (a log line, error, dashboard reading) before naming the internal cause, and state the consequence of internal mechanics instead of assuming they're familiar — e.g. what a Terraform `ignore_changes` setting causes, not just that it's set.
